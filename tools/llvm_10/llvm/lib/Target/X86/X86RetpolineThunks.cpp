...
private:
  ...
  bool PopulateLVIThunk = false;

  void createThunkFunction(Module &M, StringRef Name);
  void insertRegReturnAddrClobber(MachineBasicBlock &MBB, unsigned Reg);
  void populateThunk(MachineFunction &MF, unsigned Reg);
  void populateThunkLVI(MachineFunction &MF, unsigned Reg);
};
...
bool X86RetpolineThunks::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (!MF.getName().startswith(ThunkNamePrefix)) {
    // No easier way to do this sadly. Iterate over all functions
    // If one of the functions uses LVI then defend the retpoline
    // thunk with LVI as well.
    if (!PopulateLVIThunk){
        PopulateLVIThunk = STI->useLVIControlFlowIntegrity();
    }
...
void X86RetpolineThunks::createThunkFunction(Module &M, StringRef Name) {
  ...
  /* A bit hacky set the thunk to ExternalLinkage so the linker
     won't push the thunk to the end of the .text section. This
     guarantees that we don't end mapping a code page just to
     execute this thunk */
  Function *F =
      Function::Create(Type, GlobalValue::ExternalLinkage, Name, &M);
...
void X86RetpolineThunks::populateThunkLVI(MachineFunction &MF,
                                       unsigned Reg) {
  // Set MF properties. We never use vregs...
  MF.getProperties().set(MachineFunctionProperties::Property::NoVRegs);
  ...
  insertRegReturnAddrClobber(*CallTarget, Reg);
  CallTarget->back().setPreInstrSymbol(MF, TargetSym);
  MachineInstr *Ret = BuildMI(CallTarget, DebugLoc(), TII->get(RetOpc));

  addDirectMem(BuildMI(*CallTarget, Ret, DebugLoc(), TII->get(X86::NOT64m)), X86::RSP);
  addDirectMem(BuildMI(*CallTarget, Ret, DebugLoc(), TII->get(X86::NOT64m)), X86::RSP);
  BuildMI(*CallTarget, Ret, DebugLoc(), TII->get(X86::LFENCE));
}
