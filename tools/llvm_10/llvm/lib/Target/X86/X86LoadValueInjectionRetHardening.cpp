...
bool X86LoadValueInjectionRetHardeningPass::runOnMachineFunction(
 ...
 //                   << " *****\n");
  ...
  if (!F.hasOptNone() && skipFunction(F))
    return false;
  // We have a pass that manually blacklists functions from the kernel
  // by setting the NoLviRet attribute on the function
  ...
  for (auto &MBB : MF) {
    ...
    if (MI.getOpcode() != X86::RETQ)
      continue;
    /* IF we're also using retpolines add a different ret mitigation
       not (%rsp)
       not (%rsp)
       lfence
       ret
    ...
    if (Subtarget->useRetpolineIndirectCalls()){
       addDirectMem(BuildMI(MBB, MI, DebugLoc(), TII->get(X86::NOT64m)), X86::RSP);
       addDirectMem(BuildMI(MBB, MI, DebugLoc(), TII->get(X86::NOT64m)), X86::RSP);
       BuildMI(MBB, MI, DebugLoc(), TII->get(X86::LFENCE));
    }
    else if (ClobberReg != X86::NoRegister) {
      MBB.erase_instr(&MI);
